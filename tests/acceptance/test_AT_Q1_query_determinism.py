...
class TestScoreFormula(unittest.TestCase):
    ...
    def test_recency_boost_accepts_naive_last_access(self):
        """Naive timestamps (SQLite style) are treated as UTC."""
        from datetime import datetime, timezone

        from core.query.query_engine import recency_boost

        now = datetime.now(timezone.utc)
        last_access_naive = now.replace(tzinfo=None)

        boost = recency_boost(last_access_naive, now)
