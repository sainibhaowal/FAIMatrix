...
def test_cross_lingual_recall_uses_multilingual_bridge_and_returns_foreign_doc():
    session = _session()
    try:
        tenant_id = "tenant_cross_recall"
        graph_id = "graph-cross-recall"
        blocks = [
            EvidenceBlock.create(
                raw_id="raw-en",
                anchor=BlockAnchor(doc_type="text", char_start=0, char_end=64),
                content="Revenue planning stayed conservative for the quarter.",
                block_type="text",
            ),
            EvidenceBlock.create(
                raw_id="raw-fr",
                anchor=BlockAnchor(doc_type="text", char_start=0, char_end=64),
                content="Revenus et planification du trimestre restent stables.",
                block_type="text",
            ),
        ]
        ...
        raw_store = _RawStore(
            {
                "raw-en": b"Revenue planning stayed conservative for the quarter.",
                "raw-fr": b"Revenus et planification du trimestre restent stables.",
            }
        )
        storage_repo = _StorageFileRepo(
            [
                _StorageFileRow("raw-en", "en.txt"),
                _StorageFileRow("raw-fr", "fr.txt"),
            ]
        )
        ...
        assert top["evidence"]["raw_id"] in {"raw-fr", "raw-en"}
        phase_b = top["explain"]["phaseB_query_expansion"]
        assert phase_b["source_counts"].get("multilingual_translation", 0) >= 1
        assert phase_b["source_counts"].get("multilingual_bridge", 0) >= 1
        assert "query_fusion_summary" in top["explain"]
        assert top["explain"]["query_fusion_summary"]["query_expansion"]["expansion_count"] >= 1
    finally:
        session.close()
