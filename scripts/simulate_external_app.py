...
from store.pg.session import get_engine, get_session
...
def provision_new_tenant(tenant_name: str) -> Dict[str, str]:
    ...
    try:
        ...
        return {"tenant_id": tenant_name, "api_key": plaintext_key}
    finally:
        session.close()
...
async def simulate_external_request(credentials: Dict[str, str], faim_url: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.post(
            f"{faim_url}/v1/ingest", headers=headers, json=ingest_data
        )
        ...
        resp = await client.post(
            f"{faim_url}/v1/ingest", headers=bad_headers, json=ingest_data
        )
