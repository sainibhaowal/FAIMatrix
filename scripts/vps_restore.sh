...
docker compose -f docker-compose.yml -f docker-compose.vps.yml --profile accel stop api worker frontend caddy qdrant || true

echo "Restoring database from $DB_BACKUP..."
gunzip -c "$DB_BACKUP" | docker compose -f docker-compose.yml -f docker-compose.vps.yml --profile accel exec -T postgres psql -U faim -d faim_native
...
docker compose -f docker-compose.yml -f docker-compose.vps.yml --profile accel up -d
