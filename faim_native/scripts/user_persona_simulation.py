...
def simulate_journey():
    ...
    # 1. IDENTITY & CONTEXT
    logger.info(f"Identity: Authenticating as Tenant '{TENANT_ID}' for Graph '{GRAPH_ID}'...")
    repos = get_repos(TENANT_ID)
    session = repos['session']
    logger.info("Context: Connected to PostgreSQL Store, Qdrant Index, and Redis Cache.")
    ...
    logger.info(f"Success: Ingest complete. Packet Hash: {ingest_result.packet_hash[:12]}...")
    logger.info(f"Backend Proof: Nodes Written: {ingest_result.nodes_written}, Version: {ingest_result.graph_version}")
    ...
    for i, res in enumerate(query_result.results):
        logger.info(f"  Result {i+1}: Node ID {res['node_id'][:8]} | Score: {res['score']:.4f}")
        logger.info(f"  Physics Data: Similarity={res['score_components']['sim']:.2f}, Novelty={res['score_components']['novel']:.2f}")
    ...
    logger.info(f"Backend Proof: Merges: {evolve_result.merges}, Prunes: {evolve_result.prunes}")
    if evolve_result.status == "completed" and evolve_result.diagnostics:
        metrics = evolve_result.diagnostics.get('metrics', {})
        logger.info(f"Fractal Stats: D_hat (Order)={metrics.get('D_hat', 0):.3f}, Entropy={metrics.get('H_hat', 0):.3f}")
...
if __name__ == "__main__":
    ...
    if not os.getenv("TEST_DATABASE_URL") and not os.getenv("DATABASE_URL"):
        os.environ["DATABASE_URL"] = "postgresql://faim:testpassword123@localhost:5432/faim_test"
