...
The self-invent runtime was further hardened after the initial rollout:

- `faim_native/core/dynamics/invention_native.py`
  - now captures a real per-signature snapshot for each cycle instead of saving an empty placeholder
  - now uses a nested transaction around macro creation so concurrent identical invention attempts can fall back to the existing macro safely
- `faim_native/store/pg/repos/self_invention_state_repo.py`
  - now treats `last_event_seq` as a monotonic cursor so stale replays cannot overwrite newer progress
  - now normalizes and persists signature snapshots rather than storing empty JSON
- `faim_native/store/pg/models_faim.py`
  - now defines the `coactivations` ORM table so SQLite test fixtures and runtime metadata both include the ledger

Validation for this follow-up hardening:
