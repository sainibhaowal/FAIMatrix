...
- If `FAIM_SELF_INVENT_AFTER_UPLOAD=true`, then `FAIM_ENABLE_JOBS=true` is required
- Bounds:
...
7. In-app graph-scoped controls added:
- persisted control overrides now live in `self_evolution_state`
- the Evolution dashboard now exposes dedicated toggle buttons for:
  - self-evolve on/off
  - trigger mode
  - self-invent mode
- changes are written through `PATCH /api/v1/evolve/control`
...
- compile succeeded
- `16 passed` in the focused self-evolve/guardrail suites
- no regressions observed in S1 scope

## Outcome

Phase S1 now includes the contract + guardrails groundwork plus a real in-app control surface for graph-scoped autonomy overrides, with backward-compatible defaults, explicit operator visibility, and no runtime behavior regression by design.
