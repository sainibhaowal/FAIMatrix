...
class TestUsesEvidenceBlocks(unittest.TestCase):
    ...
    def test_vectorize_text_returns_tuple(self):
        """vectorize_text returns v_native tuple."""
        from encoding.text_vectorizer import vectorize_text

        result = vectorize_text("test query")

        # Result is (vector_list, hash, blocks)
        self.assertIsInstance(result, tuple)
        self.assertEqual(len(result), 3)
        # First element is the vector (list or tuple)
