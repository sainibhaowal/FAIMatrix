...
def _split_sentences(text: str) -> List[str]:
    chunks = [chunk.strip() for chunk in _SENTENCE_SPLIT_RE.split(text or "") if chunk.strip()]
...
def _score_span(query_text: str, span_text: str, source_rank: int, temporal_status: str | None) -> float:
    ...
    score = min(1.0, 0.45 * query_overlap + 0.30 * source_rank_score + 0.25 * temporal_consistency)
    ...
    *,
    ...
    for source_rank, item in enumerate(ranked_results):
        ...
        for sentence in _split_sentences(text)[:4]:
            span_score = _score_span(query_text, sentence, source_rank, item.get("temporal_status"))
            if span_score <= 0.0:
                continue
    ...
    spans.sort(key=lambda item: (-item.span_score, item.source_rank, item.node_id, item.text))
    return spans[:limit]
