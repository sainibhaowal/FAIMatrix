...
"""Tests for OpenTelemetry hooks in FAIM Cortex AGI Engine."""
...
from core.cortex.cortexagi.utils import otel as otel_hooks
...
class OtelHooksTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_session_and_turn_hierarchy(self):
    ...
    session_span = next(s for s in spans if s.name == "faim_cortex.session")
  ...
  async def test_interleaved_steps_tracing(self):
    ...
    parent_step_span = next(
        s
        for s in spans
        if s.name == "faim_cortex.step.1"
        and s.attributes.get("faim_cortex.step.trajectory_id") == "parent_traj"
    )
    sub_step_span = next(
        s
        for s in spans
        if s.name == "faim_cortex.step.1"
        and s.attributes.get("faim_cortex.step.trajectory_id") == "sub_traj"
    )
  ...
  async def test_step_failure_tracing(self):
    ...
    spans = self.exporter.get_finished_spans()
    step_span = next(s for s in spans if s.name == "faim_cortex.step.1")
    ...
    self.assertEqual(
        step_span.attributes.get("faim_cortex.step.status"), "ERROR"
    )
  ...
  async def test_auto_cleanup_remaining_spans(self):
    ...
    step1_span = next(
        ...
        for s in spans
        if s.name == "faim_cortex.step.1"
        and s.attributes.get("faim_cortex.step.trajectory_id") == "traj"
    )
    self.assertGreater(step1_span.end_time, 0)

    # Verify Step 2 was closed during Turn end
    step2_span = next(s for s in spans if s.name == "faim_cortex.step.2")
