...
"""OpenTelemetry Tracing Hooks for FAIM Cortex AGI Engine.
...
try:
  ...
  raise ImportError(
      ...
      "Please install them (e.g., pip install faim-cortex-agi[otel])."
  ) from e


def _get_tracer() -> Any:
  return trace.get_tracer("faim-cortex-agi-sdk")
...
class OTelSessionStartHook(hooks.OnSessionStartHook):
  ...
  async def run(self, context: hooks.HookContext, data: None) -> None:
    ...
    span = _get_tracer().start_span("faim_cortex.session")
...
class OTelPreTurnHook(hooks.PreTurnHook):
  ...
  def __init__(self, agent_name: str = "FAIMCortexAGI"):
...
class OTelPreStepHook(hooks._PreStepHook):  # pylint: disable=protected-access
  ...
  async def run(self, context: hooks.HookContext, data: types.Step) -> None:
    ...
    if is_main:
      ...
      if not subagent_span:
        ...
        subagent_span.set_attribute(
            "faim_cortex.subagent.trajectory_id", trajectory_id
        )
    ...
    span = _get_tracer().start_span(
        f"faim_cortex.step.{data.step_index}", context=parent_ctx
    )
    span.set_attribute("faim_cortex.step.index", data.step_index)
    span.set_attribute("faim_cortex.step.trajectory_id", trajectory_id)
  ...
  async def run(self, context: hooks.HookContext, data: types.Step) -> None:
    ...
    if span:
      if span.is_recording():
        span.set_attribute("faim_cortex.step.type", data.type.value)
        span.set_attribute("faim_cortex.step.status", data.status.value)
...
def get_otel_hooks(agent_name: str = "FAIMCortexAGI") -> list[hooks.Hook]:
