...
from core.cortex.cortexagi.hooks import hooks as hooks_mod
...
from core.cortex.cortexagi.triggers import triggers as triggers_mod

DEFAULT_APP_DATA_DIR = (
    (pathlib.Path("~") / ".gemini" / "faim_cortex").expanduser().resolve()
)
...
class BaseLocalAgentConfig(connection.AgentConfig):
  ...
  def _get_or_create_save_dir(self) -> str:
    ...
    if save_dir is None:
      save_dir = tempfile.mkdtemp(prefix="faim_cortex_")
...
class LocalAgentConfig(BaseLocalAgentConfig):
  ...
  def create_strategy(
    ...
    from core.cortex.cortexagi.connections.local import local_connection  # pylint: disable=g-import-not-at-top
