...
from core.cortex.cortexagi.proto import localharness_pb2
from core.cortex.cortexagi import types
from core.cortex.cortexagi.connections.local import event_processor
from core.cortex.cortexagi.connections.local import local_connection
from core.cortex.cortexagi.connections.local import local_connection_config
from core.cortex.cortexagi.connections.local import test_utils
from core.cortex.cortexagi.hooks import hook_runner
from core.cortex.cortexagi.hooks import hooks as hooks_base
from core.cortex.cortexagi.hooks import policy
from core.cortex.cortexagi.models import DEFAULT_MODEL
from core.cortex.cortexagi.tools import tool_runner
from core.cortex.cortexagi.types import QuestionResponse
...
class LocalConnectionTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_receive_steps_system_error(self):
    ...
    # receive_steps should raise FAIMCortexAGIConnectionError when it
    # encounters the system error step.
    with self.assertRaisesRegex(
        types.FAIMCortexAGIConnectionError, "Fatal system failure"
    ):
  ...
  async def test_receive_steps_system_error_401(self):
    ...
    # encounters the system error step.
    with self.assertRaisesRegex(
        types.FAIMCortexAGIConnectionError, "Unauthorized access"
    ):
      async for _ in harness.conn.receive_steps():
        pass
  ...
  async def test_receive_steps_system_error_429(self):
    ...
    steps = []
    with self.assertRaisesRegex(
        types.FAIMCortexAGIExecutionError,
  ...
  async def test_receive_steps_trajectory_error(self):
    ...
    steps = []
    with self.assertRaisesRegex(
        types.FAIMCortexAGIExecutionError, "Trajectory execution failed"
    ):
      async for step in harness.conn.receive_steps():
        steps.append(step)
  ...
  async def test_turn_hook_deny(self):
    ...
    with self.assertRaises(types.FAIMCortexAGIExecutionError) as ctx:
      async for _ in harness.conn.receive_steps():
        pass
  ...
  async def test_cancel_e2e_raises_cancelled_error(self):
    """Verifies programmatic cancel raises FAIMCortexAGICancelledError."""
    harness = test_utils.TestLocalHarness(
        test_case=self,
        process=self.mock_process,
        tool_runner=self.tool_runner,
    ...
    # Verify yielded step and ensure FAIMCortexAGICancelledError propagates
    self.assertEqual(len(steps), 1)
    self.assertEqual(steps[0].content, "I'm working")
    self.assertIsInstance(receive_error, types.FAIMCortexAGICancelledError)
...
class LocalConnectionStrategyConfigTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.patcher = mock.patch(
        "core.cortex.cortexagi.connections.local.local_connection._get_default_binary_path",
...
class LocalConnectionStrategyApiKeyTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_raises_without_api_key(self):
    ...
    strategy = self._make_strategy(models=models)
    with self.assertRaises(types.FAIMCortexAGIValidationError) as ctx:
      async with strategy:
        pass
  ...
  async def test_accepts_env_var_api_key(self, mock_popen):
    ...
    mock_popen.return_value = mock_proc
    strategy = self._make_strategy()
    # Should not raise FAIMCortexAGIValidationError; it will raise RuntimeError
...
class GetDefaultBinaryPathTest(unittest.TestCase):
  ...
  @mock.patch.dict("os.environ", {"FAIM_CORTEX_HARNESS_PATH": "/env/path"})
  ...
  def test_returns_metadata_distribution_path(self, mock_exists, mock_dist):
    ...
    mock_file.__str__.return_value = "faim_native/core/cortex/cortexagi/bin/localharness"
    mock_file.locate.return_value = (
        "/site-packages/faim_native/core/cortex/cortexagi/bin/localharness"
    )
    ...
    path = _get_default_binary_path()
    self.assertEqual(path, "/site-packages/faim_native/core/cortex/cortexagi/bin/localharness")
    mock_dist.assert_called_once_with("faim-cortex-agi")
...
class LocalConnectionUnexpectedCloseTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_unexpected_ws_close_surfaces_stderr(self):
    ...
    verify an FAIMCortexAGIConnectionError with stderr content is queued.
    """
    ...
    self.assertIsInstance(item, types.FAIMCortexAGIConnectionError)
