...
class LiteRTConnectionStrategy(LocalOpenAIConnectionStrategy):
  ...
  def _configure_litert_logging(self) -> None:
    ...
    try:
      ...
      is_debug = logging.getLogger().isEnabledFor(
          logging.DEBUG
      ) or logging.getLogger("core.cortex.cortexagi").isEnabledFor(logging.DEBUG)
  ...
  def _validate_hardware(self) -> None:
    ...
    if os.environ.get("FAIM_CORTEX_ALLOW_CPU") == "1" or self._backend in (
    ...
    if not has_gpu:
      logging.warning(
          ...
          " environment variable FAIM_CORTEX_ALLOW_CPU=1 or backend='cpu' in"
          " config."
      )
