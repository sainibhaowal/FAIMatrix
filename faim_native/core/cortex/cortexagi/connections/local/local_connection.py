...
"""Local connection for the FAIM Cortex AGI Engine."""
...
from core.cortex.cortexagi.connections import connection
from core.cortex.cortexagi.connections.local import event_processor
from core.cortex.cortexagi.hooks import hook_runner as h_runner
from core.cortex.cortexagi.tools import tool_runner as t_runner
    ...
    try:
      if self.is_idle and self._processor.step_queue.empty():
        if self._client_cancelled:
          raise types.FAIMCortexAGICancelledError()
        return
      ...
      while True:
        ...
        if (
          ...
          if http_code in (400, 401, 403):
            raise types.FAIMCortexAGIConnectionError(
    ...
    try:
      ...
      if self._disconnecting:
        ...
        await self._processor.step_queue.put(
            types.FAIMCortexAGIConnectionError(error_msg)
        )

    except Exception as e:  # pylint: disable=broad-except
      logging.exception("Error in reader loop: %s", e)
      await self._processor.step_queue.put(
          types.FAIMCortexAGIConnectionError(f"Error in reader loop: {e}")
      )
    finally:
...
def _get_sdk_version() -> str:
  """Returns the version of the FAIM Cortex AGI Engine."""
  try:
    return importlib.metadata.version("faim-cortex-agi")
...
def _get_default_binary_path_external() -> str:
  ...
  if harness_path := os.environ.get("FAIM_CORTEX_HARNESS_PATH"):
    return harness_path

  # 2. Try importlib.metadata (Robust wheel discovery)
  # This is immune to sys.path shadowing by a local repository directory.
  try:
    dist = importlib.metadata.distribution("faim-cortex-agi")
    if dist.files:
      for f in dist.files:
        normalized_path = str(f).replace("\\", "/")
        if normalized_path.endswith((
            "faim_native/core/cortex/cortexagi/bin/localharness",
            "faim_native/core/cortex/cortexagi/bin/localharness.exe",
        )):
  ...
  try:
    # Using 'core.cortex.cortexagi' as the package name.
    # This assumes the binary is located at faim_native/core/cortex/cortexagi/bin/localharness
    ...
    binary_path = str(
        importlib.resources.files("core.cortex.cortexagi").joinpath(suffix)
    )
    if os.path.exists(binary_path):
      return binary_path
  ...
  raise RuntimeError(
      ...
      "FAIM_CORTEX_HARNESS_PATH environment variable, or ensure it is in your "
    ...
    for m in self._models:
      if m.endpoint:
        try:
          ...
          raise types.FAIMCortexAGIValidationError(str(e)) from e
      else:
        raise types.FAIMCortexAGIValidationError(
