...
"""Tests for FAIM Cortex AGI Engine Pydantic type definitions.

Validates model construction, validation, immutability, forward compatibility,
and the FAIMCortexAGIValidationError wrapper.
"""
...
from core.cortex.cortexagi.conversation import conversation
...
class FAIMCortexAGIValidationErrorTest(unittest.TestCase):
  """Validates the FAIMCortexAGIValidationError wrapper."""
  ...
  def test_basic_construction(self):
    ...
    """
    err = types.FAIMCortexAGIValidationError("bad input")
  ...
  def test_from_pydantic(self):
    ...
    wrapped = types.FAIMCortexAGIValidationError._from_pydantic(err)
    self.assertIn("name", wrapped.message)
    self.assertGreater(len(wrapped.errors), 0)

  def test_is_exception(self):
    """Verifies that FAIMCortexAGIValidationError is a proper Exception.
    ...
    """
    err = types.FAIMCortexAGIValidationError("test")
  ...
  def test_with_errors_list(self):
    ...
    err = types.FAIMCortexAGIValidationError("validation failed", errors=errors)
...
class FAIMCortexAGIConnectionErrorTest(unittest.TestCase):
  """Validates the FAIMCortexAGIConnectionError hierarchy."""

  def test_is_exception(self):
    """Verifies that FAIMCortexAGIConnectionError is a proper Exception."""
    err = types.FAIMCortexAGIConnectionError("connection failed")
    self.assertIsInstance(err, Exception)

  def test_message(self):
    """Verifies that the message is stored and retrievable."""
    err = types.FAIMCortexAGIConnectionError("timeout")
