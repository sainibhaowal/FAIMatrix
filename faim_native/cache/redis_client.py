...
class RedisCache:
    ...
    def set_json(
        self, key_type: str, key: str, value: Any, ttl: int = DEFAULT_TTL
    ) -> bool:
    ...
    def check_rate_limit(
        self, endpoint: str, limit: int = 100, window_seconds: int = 60
    ) -> bool:
...
class GlobalRateLimiter:
    ...
    def check_rate_limit(
        self, key: str, limit: int = 10, window_seconds: int = 60
    ) -> bool:
        """Check if request is within rate limit."""
        client = _get_client()
        if client is None:
