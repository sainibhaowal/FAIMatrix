...
# Local imports last
from faim.pipeline.speed.spec import FaimSpeedProfile, SpeedBudget, get_speed_budget
...
try:  # optional GPU backend (P4.5)
    from faim.pipeline.speed.gpu_backend import GPUBackendConfig, GPUSearchBackend
...
class _GraphVectors:
    ...
    _gpu_backend: Optional["GPUSearchBackend"] = field(
    ...
    def add_or_update(
        self, node_id: NodeId, vec: Union[Sequence[float], np.ndarray]
    ) -> int:
        """
    ...
    def _ensure_gpu_backend_synced(self) -> Optional["GPUSearchBackend"]:
        if (
            not _GPU_BACKEND_AVAILABLE
            or GPUSearchBackend is None
            or GPUBackendConfig is None
        ):
            return None
    ...
    def _top_k_gpu(self, q: np.ndarray, k_eff: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        scores_vals, idxs_gpu = torch.topk(
            scores_gpu, k=k_eff, largest=True, sorted=False
        )
...
class VectorBank:
    ...
    Typical usage:
        ...
        from faim.pipeline.speed.spec import get_speed_budget
