...
class WriteQueue:
    ...
    def flush(self, timeout: Optional[float] = None) -> bool:
        """Wait for all currently enqueued tasks to be committed.

        This is equivalent to waiting for the highest seq_id that was
        enqueued before this call to be committed.

        Args:
            timeout: Maximum time to wait. None means wait indefinitely.

        Returns:
            True if all tasks were committed; False if timeout elapsed.
        """
        with self._lock:
            # Get the current highest seq_id minus 1 (since _next_seq is the NEXT one)
            ...
            if self._last_committed_seq >= target_seq:
                return True

        # Wait outside lock
        return self.wait_until_committed(target_seq, timeout=timeout)

    def drain(self) -> List[WriteTask]:
        """Remove and return all pending tasks from the queue.

        This is useful for safe shutdown when you need to persist
        remaining tasks through an alternative mechanism.

        Returns:
            List of all pending WriteTask items.
        """
        with self._lock:
