...
class GPUSearchBackend:
    ...
    def top_k(
        ...
        if q.shape[0] != self._dim:
            raise ValueError(f"Query dim mismatch: expected {self._dim}, got {q.shape[0]}")
    ...
    def _top_k_mirror(
        ...
        scores_vals, idxs_gpu = torch.topk(scores_gpu, k=k_eff, largest=True, sorted=True)
    ...
    def _top_k_streaming(
        ...
        N = self._n_rows
        chunk_rows = max(1, int(self._stream_chunk_rows or self._config.stream_chunk_rows or 250_000))
        device = self._config.device
        ...
        for start in range(0, N, chunk_rows):
            ...
            if scores_chunk.shape[0] <= k_eff:
                ...
                local_idx = np.argpartition(scores_chunk, -k_eff)[-k_eff:].astype(np.int64)
