...
class PostgresStore(FAIMStore, PayloadStore, EventJournal):
    ...
    def put(self, graph_id: GraphId, record: NodeRecord) -> None:
        ...
        if existing:
            ...
            existing.parents = [{"parent_id": str(p.parent_id), "fraction": p.fraction} for p in record.parents]
            ...
            new_node = NodeStorage(
                ...
                parents=[{"parent_id": str(p.parent_id), "fraction": p.fraction} for p in record.parents],
    ...
    def iter_events(self, graph_id: GraphId, since: Optional[datetime.datetime] = None) -> Iterator[JournalEvent]:
    ...
    def _row_to_record(self, row: NodeStorage) -> NodeRecord:
        ...
        parents = [ParentRef(parent_id=NodeId(p["parent_id"]), fraction=p["fraction"]) for p in (row.parents or [])]
    ...
    def upsert_node(self, node: NodeRecord) -> None:
        ...
        # Need graph_id. Try to infer or use default.
        # This implementation requires context which base store assumes.
        # We rely on existing node to get graph_id if possible.
        existing = self._db.query(NodeStorage).filter(NodeStorage.node_id == str(node.node_id)).first()
        if not existing:
            raise ValueError("Cannot upsert node without graph_id context. Use put() with explicit graph_id.")
    ...
    def find_node_id_by_payload_ref(self, graph_id: GraphId, payload_ref: PayloadRef) -> Optional[NodeId]:
