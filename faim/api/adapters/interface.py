...
# LLM removed - stub for compatibility
def get_llm():
    return None
...
def retrieve_context(graph_id: str, user_text: str, limit: int = 8) -> Tuple[List[Dict[str, Any]], str]:
    """
    ...
    for nid, score in hits[:limit]:
        ...
        kind = "fact" if isinstance(payload, str) and payload.strip().startswith(_FACT_PREFIX) else "node"
        why = "engine_retrieve" if _try_engine_retrieve(gid, query, k=1) else "fallback_keyword_scan"
...
def store_chat_and_facts(graph_id: str, user_text: str, assistant_text: str) -> Tuple[List[str], List[str]]:
    """
