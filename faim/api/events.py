        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    yield format_sse(
                        SseEvent(event="ping", data={"ts": time.time(), "graph_id": self.graph_id})
                    )
        finally:
...
async def emit_fig_delta(graph_id: str, delta: dict[str, Any]) -> int:
    return await BUS.publish(
        graph_id, "fig_delta", {"graph_id": graph_id, "ts": time.time(), "delta": delta}
    )
...
async def emit_metrics(graph_id: str, cards: dict[str, Any]) -> int:
    return await BUS.publish(
        graph_id, "metrics", {"graph_id": graph_id, "ts": time.time(), "cards": cards}
    )
