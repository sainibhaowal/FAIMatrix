...
async def process_file(
    ...
    try:
        ...
        try:
            ...
            result = extract_text(tmp_path, filename)
        ...
        batch.files[filename].message = "Creating memory nodes..."
        batch.files[filename].progress_percent = 60
...
async def process_batch(batch_id: str, files: List[UploadFile], graph_id: str):
    """
    ...
    async def process_with_limit(file: UploadFile):
        async with semaphore:
            await process_file(batch_id, file, graph_id)
...
async def upload_batch(
    ...
    asyncio.create_task(process_batch(batch_id, files, graph_id))
