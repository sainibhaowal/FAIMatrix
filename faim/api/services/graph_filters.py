...
Usage:
    from faim.api.services.graph_filters import filter_snapshot, FilterOptions
...
async def get_filtered_snapshot(
    ...
    try:
        from faim.api.services.graphs import _build_snapshot

        snapshot = _build_snapshot(graph_id)
    except ImportError:
        # Fallback: build manually from store
        try:
            from faim.config.backends import get_store
