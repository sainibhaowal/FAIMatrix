...
from faim.api.adapters.adapter import (
    format_node_for_ui,
...
from faim.api.adapters.interface import add_fragment, get_node_detail

# --- Retrieval debug
from faim.api.adapters.retrieve.context import naive_used_nodes
from faim.api.auth.auth import verify_graph_access
from faim.api.services.events import emit_fig_delta

# --- Models (stable API contract)
from faim.api.services.models import (
    GraphMetrics,
    GraphSubgraph,
    IngestFileResult,
    IngestResponse,
...
def _build_snapshot_from_store(graph_id: str) -> GraphSnapshot:
    ...
    for n in nodes_raw:
        ...
        for pid_raw, w in parents:
            ...
            if pid:
                out_links.append({"source": pid, "target": nid, "rel": "inherits", "weight": float(w)})
    ...
    out_links.sort(key=lambda e: (_as_str(e.get("source")), _as_str(e.get("target")), _as_str(e.get("rel"))))
...
def _build_snapshot(graph_id: str) -> GraphSnapshot:
    ...
    links_out.sort(key=lambda e: (_as_str(e.get("source")), _as_str(e.get("target")), _as_str(e.get("rel"))))
    return GraphSnapshot(
        graph_id=graph_id,
        nodes=nodes_out,
        links=links_out,
...
def _scan_nodes_any(graph_id: str, limit: int, order: str) -> List[Any]:
    ...
    try:
        ...
        if ord_norm == "recent":
            nodes.sort(key=lambda n: _as_float(getattr(n, "last_used_at", 0.0), default=0.0), reverse=True)
        else:
...
# -----------------------------------------------------------------------------
# CONTEXT SEARCH — Semantic similarity search for UI
# -----------------------------------------------------------------------------
...
def api_context_search(
    ...
    """Semantic context search - find related memory nodes."""
    q = (query or "").strip()
    if not q:
        raise HTTPException(status_code=400, detail="query is required")
    kk = _clamp_int(k, 1, 50, 10)
    used_nodes, _ = naive_used_nodes(graph_id, q, k=kk)
...
def recent_used_nodes(graph_id: str, limit: int = 20):
    ...
    from faim.api.adapters.interface import get_recent_used_nodes
...
def api_node_detail(graph_id: str, node_id: str):
    # Normalize node_id: strip graph_id prefix if frontend sent it
    # Frontend may send "U:f0bffc04:doc_xxx" but DB has "doc_xxx"
    normalized_node_id = node_id
    if ":" in node_id:
        parts = node_id.split(":")
        # If format is "U:graphid:nodeid" or similar, take the last part
        if len(parts) >= 3:
            normalized_node_id = parts[-1]
        elif len(parts) == 2 and parts[0].startswith("U"):
            # Might be "U:nodeid" - keep as is or take second part
            pass

    raw = get_node_detail(graph_id, normalized_node_id)
    if raw is None:
        # Fallback: try original node_id in case normalization was wrong
...
def api_metrics(graph_id: str):
    ...
    retrieve_p50_ms = _num(
        m.get("retrieve_p50_ms", lat.get("retrieve_p50_ms", lat.get("p50", lat.get("p50_ms", 0.0)))),
        0.0,
    )
    retrieve_p95_ms = _num(
        m.get("retrieve_p95_ms", lat.get("retrieve_p95_ms", lat.get("p95", lat.get("p95_ms", 0.0)))),
        0.0,
    )
...
def api_galaxies(graph_id: str, sample: int = Query(600, ge=50, le=5000)):
    ...
    galaxies = [{"id": k, "label": k.title(), "count": int(v)} for k, v in counts.items() if int(v) > 0]
