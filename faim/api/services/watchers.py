...
from faim.api.services.events import BUS, emit_fig_delta, emit_metrics
...
DEFAULT_GRAPH = os.getenv("FAIM_DEFAULT_GRAPH", "")  # Must be set explicitly or derived from user
...
# NOTE: Evolution and Invention schedulers have been moved to FAIMEngine.
# They are now managed internally by the Engine (start on init, stop on shutdown).
# See faim/core/evolution.py::EvolutionScheduler and faim/core/invention.py::InventionScheduler.
...
def start_watchers(loop: asyncio.AbstractEventLoop):
    ...
    NOTE: Evolution and Invention schedulers are now managed by FAIMEngine
    internally. They start when Engine is created and stop on shutdown.
    """
