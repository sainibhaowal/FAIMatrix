...
test.describe('Real backend — graph surface', () => {
  test('FIG page loads and displays real node count from live backend', async ({ page }) => {
    await injectSession(page);
    await page.goto('/dashboard/graph');

    // Page chrome is always visible — use nth(2) to target the graph-page header span
    await expect(page.getByText('FIG View').nth(2)).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.locator('[data-testid="fig-metrics-nodes"]').or(
    ...
    const statsText = await page.locator('text=/Nodes/').first().isVisible({ timeout: 10000 });
    expect(statsText).toBe(true);
  });

  test('graph surface response contains real nodes — intercepted and counted', async ({ page }) => {
    await injectSession(page);
    ...
    console.log(`[real-test] Surface: ${surfaceNodeCount} nodes, ${surfaceEdgeCount} edges`);
  });

  test('graph snapshot metadata reflects real backend version and hash', async ({ page }) => {
    await injectSession(page);
    ...
    await page.route('**/api/v1/graph/surface**', async (route) => {
      const response = await route.fetch();
      const body = await response.json() as {
        snapshot: { graph_hash: string; graph_version: number; graph_id: string };
      };
    ...
    const surfaceResponsePromise = page.waitForResponse('**/api/v1/graph/surface**');
    await page.goto('/dashboard/graph');
    ...
    console.log(`[real-test] Snapshot: v${graphVersion}, id=${graphId}, hash="${snapshotHash}"`);
  });
});
...
test.describe('Real backend — neighborhood expansion', () => {
  test('neighborhood endpoint returns real nodes for a seed from the surface', async ({ page }) => {
    await injectSession(page);
    ...
    const result = await page.evaluate(
      async ({ jwt, graphId, nodeId }: { jwt: string; graphId: string; nodeId: string }) => {
        if (!nodeId) return null;
        const resp = await fetch(
          `/api/v1/graph/neighborhood?graph_id=${graphId}&node_id=${nodeId}&depth=1`,
          { headers: { Authorization: `Bearer ${jwt}` } },
        );
        return resp.ok ? (await resp.json()) as { nodes: unknown[]; edges: unknown[] } : null;
      },
  ...
  test('neighborhood merge result is additive — no duplicates', async ({ page }) => {
    await injectSession(page);
    ...
    const nbhd = await page.evaluate(
      async ({ jwt, graphId, nodeId }: { jwt: string; graphId: string; nodeId: string }) => {
        ...
        return resp.ok ? (await resp.json()) as { nodes: { node_id: string }[] } : null;
      },
      { jwt: E2E_JWT, graphId: E2E_GRAPH_ID, nodeId: seedNodeId },
    );
...
test.describe('Real backend — timeline and events', () => {
  test('events/latest returns non-zero seq from real event journal', async ({ page }) => {
    await injectSession(page);
  ...
  test('timeline drawer opens and shows real event entries from backend', async ({ page }) => {
    await injectSession(page);
    ...
    await expect(page.getByText('Live timeline')).toBeVisible({ timeout: 5000 });
  ...
  test('live sync poll hits real events/latest endpoint', async ({ page }) => {
    ...
    await page.getByTitle('Timeline').click();
    await expect(page.getByText('Live timeline')).toBeVisible({ timeout: 8000 });
...
test.describe('Real backend — search overlay with real nodes', () => {
  test('search overlay finds and displays real node titles from backend', async ({ page }) => {
    await injectSession(page);
    ...
    await expect(page.getByPlaceholder(/Search nodes/i)).toBeVisible({ timeout: 5000 });
    ...
    const resultCount = await page
      ...
      .count();
    console.log(`[real-test] Search results visible: ${resultCount}, first title: ${realNodeTitle}`);

    // Close with Escape
    await page.keyboard.press('Escape');
    await expect(page.getByPlaceholder(/Search nodes/i)).not.toBeVisible({ timeout: 3000 });
  });
});
...
test.describe('Real backend — inspector + neighborhood via search', () => {
  test('select node via search overlay opens inspector with real node data', async ({ page }) => {
    await injectSession(page);
    ...
    const firstResult = page.locator('button, [role="option"]').filter({
    ...
    if (firstResultVisible) {
      ...
      await expect(page.getByText('Inspector').or(page.getByTitle('Inspector'))).toBeVisible({
        timeout: 5000,
      });
      console.log(`[real-test] Inspector opened for node: ${firstNodeId.slice(0, 16)}`);
    } else {
      ...
      console.log('[real-test] No clickable search result; inspector shows placeholder as expected');
    }
  });

  test('neighborhood expand button calls real backend and returns data', async ({ page }) => {
    await injectSession(page);
    ...
    const nbhdResult = await page.evaluate(
      async ({
        ...
        return resp.ok
          ? (await resp.json()) as { nodes: unknown[]; edges: unknown[]; seed_node_id: string }
          : null;
      },
    ...
    expect((nbhdResult as { seed_node_id: string }).seed_node_id).toBe(firstNodeId);
    expect((nbhdResult as { nodes: unknown[] }).nodes.length).toBeGreaterThan(0);
    console.log(
