...
async function writeMemory(idempotencyKey: string, text: string): Promise<void> {
  ...
  if (resp.status === 409) {
    ...
    console.log(`[e2e-setup] ${idempotencyKey}: already seeded (409), skipping`);
    return;
  }
  if (!resp.ok) {
    const body = await resp.text();
    throw new Error(`memory/write failed (${resp.status}): ${body.slice(0, 200)}`);
  }
  const data = (await resp.json()) as { nodes_written: number };
  console.log(`[e2e-setup] ${idempotencyKey}: nodes_written=${data.nodes_written}`);
}
...
export default async function globalSetup(): Promise<void> {
  ...
  if (!health.ok) {
    throw new Error(`Backend not healthy at ${E2E_BACKEND_URL} (${health.status})`);
  }
  ...
  if (!surfaceCheck.ok) {
    throw new Error(`JWT auth failed (${surfaceCheck.status}) — check NEXTAUTH_SECRET`);
  }
  ...
  if (surface.nodes.length === 0) {
    throw new Error('E2E graph has 0 nodes after seeding — check memory/write pipeline');
  }
  ...
  const ev = (await evResp.json()) as { last_seq: number; last_kind: string | null };
