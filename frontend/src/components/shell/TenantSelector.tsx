...
export function TenantSelector() {
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                const apiBase = (process.env.NEXT_PUBLIC_FAIM_API_BASE_URL || "/api/v1").replace(/\/+$/, "");
    ...
    if (loading) return <div className="text-xs text-white/40 px-4 py-2">Loading context...</div>;

    // Fallback if no orgs found (e.g. backend down or new user)
    if (orgs.length === 0) {
        return (
            <div className="mb-4 px-4 py-2">
                <div className="text-xs text-white/40 uppercase tracking-widest font-semibold mb-1">Context</div>
            ...
            <div className="group relative">
                <button className="flex w-full items-center justify-between rounded-xl border border-white/5 bg-transparent px-3 py-2 text-left text-sm text-slate-400 hover:border-white/10 hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        <span className="truncate">{selectedProject?.name || "Default Project"}</span>
                    </div>
                    <ChevronDown size={14} className="opacity-50" />
                </button>
            </div>
