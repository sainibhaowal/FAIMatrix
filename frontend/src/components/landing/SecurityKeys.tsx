...
const SCOPES = [
  ...
  { name: "memory.admin", desc: "Evolve, prune, admin operations", level: "admin" },
];

const SECURITY_LAYERS = [
  { name: "Tenant Isolation", desc: "Crypto-isolated per tenant at middleware level", icon: "\uD83D\uDD12" },
  { name: "JWT + API Key", desc: "Dual auth: session tokens or scoped API keys", icon: "\uD83D\uDDDD\uFE0F" },
  { name: "Rate Limiting", desc: "Per-tenant, per-route rate limits enforced", icon: "\u26A1" },
  { name: "Security Headers", desc: "HSTS, CSP, X-Frame-Options, X-Content-Type", icon: "\uD83D\uDEE1\uFE0F" },
  { name: "Request Tracing", desc: "Every request gets a unique X-Request-Id", icon: "\uD83D\uDD0D" },
  { name: "CORS Policy", desc: "Strict origin whitelist, no wildcard", icon: "\uD83C\uDF10" },
];
...
export default function SecurityKeys() {
  ...
  return (
    <section id="security" className="py-28 px-4 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative">
          ...
          {KEY_LIFECYCLE.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                <div className={`inline-flex p-2 rounded-lg ${c.bg} ${c.text} mb-3`}>
                  {item.icon}
                </div>
                <h3 className="text-white font-bold text-sm mb-1">{item.step}</h3>
                <p className="text-slate-400 text-xs mb-2">{item.description}</p>
                <p className="text-slate-600 text-[11px] font-mono">{item.detail}</p>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            <h3 className="text-lg font-bold text-white mb-6">Granular Scopes</h3>
          ...
          <motion.div
            ...
            className="p-8 rounded-2xl border border-slate-800 bg-slate-900/30"
          >
            <h3 className="text-lg font-bold text-white mb-6">6-Layer Middleware Stack</h3>
            ...
            <div className="mt-6 p-4 rounded-lg bg-slate-950 border border-slate-800">
              <p className="text-[10px] text-slate-600 font-mono uppercase tracking-wider mb-2">Middleware Order</p>
              <p className="text-xs font-mono text-slate-400">
                SecurityHeaders \u2192 JWT \u2192 TenantAuth \u2192 RateLimit \u2192 RequestId \u2192 CORS
              </p>
            </div>
          </motion.div>
        </div>
