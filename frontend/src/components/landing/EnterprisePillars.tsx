...
export default function EnterprisePillars() {
  return (
    <section className="py-12 sm:py-28 px-4 bg-slate-950 relative overflow-hidden border-y border-slate-900">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {PILLARS.map((pillar, idx) => (
            <motion.div
              ...
              className="p-4 sm:p-6 rounded-2xl border border-slate-800 bg-slate-900/10 hover:border-slate-700 hover:bg-slate-900/30 transition-all group"
            >
              <div
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-${pillar.color}-500/10 flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform`}
              >
                <div className={`text-${pillar.color}-400`}>{pillar.icon}</div>
              </div>
              <h3 className="text-white font-bold mb-1">{pillar.title}</h3>
              <p
                className={`text-[10px] font-bold uppercase tracking-widest text-${pillar.color}-500/80 mb-3`}
              >
                {pillar.subtitle}
              </p>
