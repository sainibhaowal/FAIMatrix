...
const LAYERS = [
  {
    ...
    items: [
      { name: "App + LLM Integration", desc: "Use FAIM directly or add an LLM as a translator on top" },
      { name: "REST Query + Answer API", desc: "Deterministic retrieval, explain payloads, and citation-first answers" },
      { name: "API Keys + Tenant Isolation", desc: "Scoped access for multi-tenant production systems" },
      { name: "Dashboard + FIG View", desc: "Graph exploration, rebuild jobs, and operational visibility" },
    ],
  },
  {
    id: "engine",
    ...
    items: [
      { name: "Native Core + Invariants", desc: "256-d vectors, inheritance, dedup, evolution, and 8 invariant checks" },
      { name: "Lexical + Canonical Layers", desc: "Representation V2, canonical semantics, aliases, phrases, and sparse scoring" },
      { name: "Graph + Knowledge Layers", desc: "Diffusion, semantic edges, domain knowledge, and multilingual concept links" },
      { name: "Answer-Ready Retrieval", desc: "Deterministic reranking, multimodal evidence, and extractive answer synthesis" },
    ],
  },
] as const;
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        >
          <p className="text-slate-400 text-sm leading-relaxed max-w-3xl mx-auto">
            ...
            It stores, links, retrieves, ranks, and answers from deterministic graph-native memory.{" "}
            <span className="text-white font-medium">The application layer is optional orchestration.</span>{" "}
            Add an LLM, keep it out, or swap it later. FAIM still owns the memory truth,
            retrieval logic, and citation chain.
          </p>
        </motion.div>
      </div>
    </section>
