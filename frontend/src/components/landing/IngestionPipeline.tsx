...
const PIPELINE_STAGES = [
  {
    ...
    color: "cyan",
    description:
      "Upload PDF, DOCX, TXT, or paste text. Multi-file batch upload supported.",
    detail:
      "File is stored immutably with SHA-256 hash. Raw bytes encrypted at rest.",
  ...
  },
  {
    ...
    color: "blue",
    description:
      "Perception router extracts EvidenceBlocks. No crude chunking \u2014 respects document structure.",
  ...
  },
  {
    ...
    color: "emerald",
    description:
      "Vectors enter the engine: parent selection, inheritance computation, antisymmetric merge scan, invariant verification.",
    detail:
      "8 invariants checked. Graph version bumped. Event journal updated. Graph hash recomputed.",
...
const DEDUP_EXAMPLE = {
  first: {
    file: "security-policy-v2.pdf",
    hash: "a4c1e7f...",
    result: "3 nodes created",
  },
  second: {
    file: "security-policy-v2.pdf",
    hash: "a4c1e7f...",
    result: "dedup_hit: true \u2192 0 nodes",
  },
};
...
export default function IngestionPipeline() {
  ...
  return (
    <section
      id="ingestion"
      className="py-28 px-4 bg-gradient-to-b from-slate-950 to-[#070a18] relative overflow-hidden"
    >
          ...
          {PIPELINE_STAGES.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`inline-flex p-2 rounded-lg ${isActive ? c.bg : "bg-slate-800/50"} mb-2`}
                >
                  <div className={isActive ? c.text : "text-slate-500"}>
                    {s.icon}
                  </div>
                </div>
                <p
                  className={`text-xs font-bold ${isActive ? c.text : "text-slate-500"}`}
                >
                  {String(i + 1).padStart(2, "0")}
                </p>
                <p
                  className={`text-sm font-medium mt-0.5 ${isActive ? "text-white" : "text-slate-400"}`}
                >
                  {s.label}
                </p>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <p className="text-slate-400 text-sm mb-6">
              Every file produces a unique packet_hash. Upload the same file
              again \u2014 FAIM detects the duplicate and returns the cached
              result. Zero new nodes.
            </p>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-lg border border-amber-500/20 bg-amber-500/[0.04]">
                ...
                <p className="text-sm text-white font-mono">
                  {DEDUP_EXAMPLE.second.file}
                </p>
                <p className="text-xs text-slate-500 font-mono mt-1">
                  packet_hash: {DEDUP_EXAMPLE.second.hash}{" "}
                  <span className="text-amber-400">(match!)</span>
                </p>
                <p className="text-xs text-amber-400 mt-1">
                  \u2192 {DEDUP_EXAMPLE.second.result}
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 rounded-lg bg-slate-950 border border-slate-800">
              ...
              <div className="space-y-1 text-xs font-mono text-slate-400">
                <p>
                  <span className="text-cyan-400">INGEST_START</span> \u2192
                  PACKET_CREATED \u2192 ENCODED
                </p>
                <p>
                  \u2192{" "}
                  <span className="text-emerald-400">WRITE_ATOMS_DONE</span>{" "}
                  \u2192 INDEX_UPSERTED
                </p>
                <p>
                  \u2192{" "}
                  <span className="text-purple-400">INGEST_PHASE_LATENCY</span>
                </p>
              </div>
            </div>
          </motion.div>
