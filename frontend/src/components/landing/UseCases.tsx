...
const USE_CASES: UseCase[] = [
  ...
  },
  {
    ...
    reason:
      "No patient data leaves the device. SHA-256 integrity for every record.",
    features: [
  ...
  },
  {
    ...
    reason:
      "Runs on minimal hardware. No cloud connectivity needed. No GPU required.",
    features: [
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {USE_CASES.map((uc, i) => (
            ...
            >
              {/* Gradient glow */}
              <div
                className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${uc.gradient} opacity-0 group-hover:opacity-[0.03] transition-opacity duration-500 pointer-events-none`}
              />

              <div className="relative z-10">
                {/* Icon */}
                <div
                  className={`inline-flex p-3 rounded-xl bg-gradient-to-br ${uc.gradient} text-white mb-5`}
                >
                  {uc.icon}
                </div>
