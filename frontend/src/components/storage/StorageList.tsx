            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-end gap-1 min-w-[100px]">
                  <div className="flex items-center gap-2">
                    {doc.fidelity_score !== undefined && (
                      <FidelityBadge score={doc.fidelity_score} />
                    )}
                    <StatusBadge status={doc.status} />
                  </div>
                  {doc.status === "completed" && doc.nodes_created !== undefined && (
                    <div className="text-[9px] text-cyan-500/80 font-mono mt-1">
                      {doc.nodes_created} neural nodes
                    </div>
                  )}
                </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <div className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${classes}`}>
      {status.toUpperCase()}
    </div>
  );
}
...
function FidelityBadge({ score }: { score: number }) {
  const colorClass = 
    score >= 95 ? "text-emerald-400 border-emerald-500/30 bg-emerald-500/5" :
    score >= 80 ? "text-cyan-400 border-cyan-500/30 bg-cyan-500/5" :
    score >= 50 ? "text-amber-400 border-amber-500/30 bg-amber-500/5" :
    "text-rose-400 border-rose-500/30 bg-rose-500/5";

  return (
    <div 
      className={`px-1.5 py-0.5 rounded border text-[9px] font-bold flex items-center gap-1 cursor-help ${colorClass}`}
      title={`Continuity Health: ${score}% of source text is preserved in the graph.`}
    >
      <div className={`w-1 h-1 rounded-full ${score >= 80 ? 'bg-current animate-pulse' : 'bg-current'}`} />
      {score}% FIDELITY
    </div>
  );
}
