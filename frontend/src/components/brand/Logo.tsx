...
function InteractiveLogo() {
  ...
  const pStrength = useMemo(
    () => ({ back: 3, mid: 8, front: 12, extras: 15 }),
    [],
  );
  ...
  const arc = (r: number, a0: number, a1: number) => {
    ...
    const p0 = {
      x: cx + r * Math.cos(toRad(a0)),
      y: cy + r * Math.sin(toRad(a0)),
    };
    const p1 = {
      x: cx + r * Math.cos(toRad(a1)),
      y: cy + r * Math.sin(toRad(a1)),
    };
  ...
  const leftTraces = [
    ...
    {
      d: `M 410 560 L 300 560 L 260 610 L 190 610`,
      tone: "neutral",
      nodes: [{ x: 190, y: 610, r: 8 }],
    },
    {
      d: `M 410 410 L 320 410 L 280 360 L 200 360`,
      tone: "neutral",
      nodes: [{ x: 200, y: 360, r: 8, hollow: true }],
    },
    {
      d: `M 360 360 L 320 320 L 260 320`,
      tone: "cool",
      nodes: [{ x: 260, y: 320, r: 7 }],
    },
    {
      d: `M 330 610 L 290 660 L 230 660`,
      tone: "cool",
      nodes: [{ x: 230, y: 660, r: 7, hollow: true }],
    },
  ] as const;
    ...
    >
      ...
      >
          ...
          <g strokeLinecap="round" strokeLinejoin="round" fill="none">
            {leftTraces.map((t, i) => (
              <g key={`L-${i}`}>
                <path
                  d={t.d}
                  stroke={t.tone === "cool" ? "#4c8dff" : "#b8c7ff"}
                  strokeOpacity={0.15}
                  strokeWidth={6}
                />
                <path
          ...
          >
            {leftTraces.map((t, i) => (
              <g key={`R-${i}`}>
                <path
                  d={t.d}
                  stroke={t.tone === "cool" ? "#f7b955" : "#ffd9b3"}
                  strokeOpacity={0.15}
                  strokeWidth={6}
                />
                <path
        ...
        <g style={pStyle(pStrength.extras)}>
          <path
            d={`M ${cx - 42} ${cy - 300} L ${cx - 20} ${cy - 330} H ${cx + 20} L ${cx + 42} ${cy - 300} Z`}
            fill="#0a1222"
            stroke="#2a3b66"
            strokeWidth={3}
            opacity={0.95}
