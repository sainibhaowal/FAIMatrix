...
/**
 * FAIM Intelligence Features Panel
 *
 * Provides UI for all 6 Intelligence features:
...
import { GovernanceDashboard } from "@/components";
...
interface IntelligenceFeaturesPanelProps {
  onInventionSelect?: (conceptId: string) => void;
}

export default function IntelligenceFeaturesPanel({ onInventionSelect }: IntelligenceFeaturesPanelProps) {
  ...
  const [evolutionResult, setEvolutionResult] = useState<FeatureResult>({
    loading: false,
    success: false,
  });
  ...
  const handleRunEvolution = useCallback(async () => {
    setEvolutionResult({ loading: true, success: false });

    try {
      const res = await fetch(`/api/ops/evolution/trigger?graph_id=${graphId}&proposal_mode=true`, {
        method: "POST",
      });

      const data = await res.json();
      setEvolutionResult({ loading: false, success: true, data });
    } catch (e: any) {
      setEvolutionResult({ loading: false, success: false, error: e.message });
    }
  }, [graphId]);
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="mt-2 max-h-20 overflow-y-auto">
              {inventionResult.data.inventions
                .map((inv: Invention, i: number) => (
                  <button 
                    key={i} 
                    onClick={() => onInventionSelect?.(inv.concept_id)}
                    className="w-full text-left p-1 rounded hover:bg-emerald-500/10 transition-colors"
                  >
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <StatusIcon result={evolutionResult} />
          </div>

          <p className="mb-3 text-[10px] text-slate-500">
            Stage self-optimization changes for review.
          </p>

          <button
            onClick={handleRunEvolution}
            disabled={evolutionResult.loading}
            className="w-full rounded-lg bg-indigo-600/20 px-3 py-1.5 text-[10px] font-medium text-indigo-300 hover:bg-indigo-600/30 disabled:opacity-50"
          >
            {evolutionResult.loading ? "Optimizing..." : "Trigger Evolution"}
          </button>

          {evolutionResult.data && (
            <p className="mt-2 text-[10px] text-emerald-400">
              ✓ {evolutionResult.data.proposals_count} changes staged for review
            </p>
          )}
        </div>
      </div>
