...
export function MetricsCards({ metrics }: { metrics: GraphMetrics }) {
  ...
  const cards = [
    { title: 'Nodes', value: node_count.toLocaleString(), hint: 'Active memory nodes in FIG' },
    { title: 'Compression Ratio (CR)', value: `${compression_ratio.toFixed(1)}×`, hint: 'Raw vs FAIM storage (higher is better)' },
    { title: 'Redundancy (R)', value: `${(redundancy * 100).toFixed(2)}%`, hint: 'Local overlap between nodes (lower is better)' },
    { title: 'Drift', value: `${(drift * 100).toFixed(2)}%`, hint: 'Recall degradation (lower is better)' },
    { title: 'Raw bytes', value: `${rawBytes}`, hint: 'Approx raw payload bytes (baseline).' },
    { title: 'FAIM bytes', value: `${faimBytes}`, hint: 'Approx compressed FAIM footprint.' },
    { title: 'Retrieve p50 / p95', value: `${p50.toFixed(1)} / ${p95.toFixed(1)} ms`, hint: 'Latency for memory lookup' },
    { title: 'Storage (raw → FAIM)', value: `${(rawBytes / 1_048_576).toFixed(1)} MB → ${(faimBytes / 1_048_576).toFixed(2)} MB`, hint: 'Actual disk footprint before/after FAIM' },
  ];
    ...
    <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-3">
      {cards.map((c) => (
        ...
        >
          ...
          >
            <div className="text-xs uppercase tracking-wide text-slate-400">{c.title}</div>
            <div className="mt-2 text-2xl font-semibold text-cyan-300">{c.value}</div>
