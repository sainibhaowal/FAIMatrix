      ...
      if (!res.ok) {
        if ([404, 405, 415].includes(res.status)) {
          ...
          for (let i = 0; i < selectedFiles.length; i += 1) {
            ...
            const chunks = chunkText(cleaned, MAX_FRAGMENT_CHARS).filter((c) => c.trim());
            let ok = true;
        ...
        <div className="mb-3 flex items-center justify-between">
          <div className="text-[0.7rem] text-slate-400">
            {selectedFiles && selectedFiles.length > 0 ? (
              <span>
                Selected{' '}
                <span className="font-semibold text-slate-100">{selectedFiles.length}</span>{' '}
                file(s)
              </span>
            ) : (
            ...
            <table className="min-w-full border-collapse text-[0.7rem]">
              ...
              <tbody>
                {lastResult.files.map((f) => (
                  <tr key={f.filename} className="border-t border-slate-800 text-slate-300">
                    <td className="px-3 py-1.5">
                      <span className="font-mono text-[0.65rem] text-slate-200">{f.filename}</span>
                    </td>
                    <td className="px-3 py-1.5">{(f.size_bytes / 1024).toFixed(1)} KB</td>
                    ...
                    <td className="px-3 py-1.5">
                      ...
                      {f.note && (
                        <span className="ml-1 text-[0.6rem] text-slate-500">{f.note}</span>
                      )}
                    </td>
                  </tr>
                ))}
