    ...
    async function loadClusters(signal: AbortSignal) {
      ...
      if (Array.isArray((body as any)?.clusters)) {
        stats = (body as any).clusters
          ...
          .filter(
            (c: ClusterStat) =>
              Number.isFinite(c.cluster_id) && Number.isFinite(c.count),
          );
        ...
        stats = Array.from(counts.entries()).map(([cluster_id, count]) => ({
          cluster_id,
          count,
        }));
      } else if (body && typeof body === "object") {
            ...
            {degError ? (
              ...
              <div className="h-32 rounded bg-slate-950/80 p-2">
                ...
                <p className="mt-1 text-[10px] text-slate-500">
                  Each bar shows how many nodes have a given degree (parents +
                  children).
                </p>
              </div>
            )}
          ...
          <div>
            ...
            {cluError ? (
              ...
              <p className="rounded border border-dashed border-slate-700 bg-slate-950/60 px-2 py-3 text-[11px] text-slate-500">
                ...
                <code className="font-mono text-[10px]">cluster_map</code> is
                wired, cluster sizes will appear here.
              </p>
            ) : (
            ...
            <p className="mt-1 text-[10px] text-slate-500">
              Later, FIG nodes can be colored by cluster id for quick visual
              structure.
            </p>
          </div>
        </div>
