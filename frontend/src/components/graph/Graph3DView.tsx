...
import { NeuralClusterGeometry } from "./NeuralClusterGeometry";
...
function buildNodeTooltip(node: any): string {
  ...
  // FAIM-Native Provenance Details
  const lines: string[] = [header];
  
  if (node.vector_hash) {
    lines.push(`🔐 Hash: ${node.vector_hash.slice(0, 16)}...`);
  }
  if (typeof node.level === "number") {
    lines.push(`📊 Level: ${node.level}`);
  }
  if (node.parents && node.parents.length > 0) {
    lines.push(`🧬 Parents: ${node.parents.length}`);
  }
  if (node.opp_signature) {
    const oppKeys = Object.keys(node.opp_signature);
    if (oppKeys.length > 0) {
      lines.push(`⚡ Antisym: ${oppKeys.length} sig`);
    }
  }
  if (typeof node.novelty === "number") {
    lines.push(`✨ Novelty: ${(node.novelty * 100).toFixed(0)}%`);
  }
  if (node.evolution_flags && node.evolution_flags.length > 0) {
    lines.push(`🔄 Flags: ${node.evolution_flags.join(", ")}`);
  }
...
export const Graph3DView = forwardRef<Graph3DRef, Graph3DViewProps>(({
  ...
  // Size state (drives ForceGraph props)
  const [size, setSize] = useState<{ w: number; h: number }>({
    w: 300,
    h: 300,
  });

  // Universe-first graphId
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Update the shared material time (handles base and non-cloned materials)
      NeuralClusterMaterial.uniforms.uTime.value = elapsed;

      // Update cloned materials and pulsing in the scene
      if (fgRef.current) {
        const scene = fgRef.current.scene();
        if (scene) {
          scene.traverse((obj: any) => {
            // Update time for shader-based pulsing
            if (obj.isPoints && obj.material && obj.material.uniforms && obj.material.uniforms.uTime) {
              obj.material.uniforms.uTime.value = elapsed;
            }
            
            // Pulse the entire group scale for organic breathing effect
            if (obj.isGroup && obj.userData && obj.userData.pulseSpeed) {
              const pulse = Math.sin(elapsed * obj.userData.pulseSpeed + obj.userData.phase) * 0.05 + 1.0;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      let u = graphId?.startsWith("U:") ? graphId : getUniverseGraphId();
      
      // Use default graph if nothing set
      if (!u || !u.startsWith("U:")) {
         u = "U:default-universe";
      }
      ...
      try {
        ...
        console.warn("[Graph3DView] fetchSubgraph failed:", err);
      }

      // Try node scan as fallback
      try {
        ...
        console.warn("[Graph3DView] fetchNodeScan failed:", err);
        // No mock fallback - show empty graph if backend unavailable
      }
    };
    ...
    >
      ...
      >
        ...
        <ForceGraph3D
          ...
          nodeThreeObject={(node: any) => NeuralClusterGeometry.createCluster(node)}
          nodeThreeObjectExtend={false}
          enableNodeDrag={enableNodeDrag ?? true}
          linkOpacity={0.3}
          // Edge width based on weight - thinner filaments for radial design
          linkWidth={(l: any) =>
            l.rel === "tether" ? 0.05 : 0.15
          }
          // Edge color: neural filaments with cyan/slate tones
          linkColor={(l: any) => {
            if (l.rel === "tether") return "rgba(148, 163, 184, 0.1)";
