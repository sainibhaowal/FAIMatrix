...
export default function AGIFeaturesPanel() {
  ...
  const { graphId: userGraphId, projectId, isLoading: userLoading } = useUserIds();
  const graphId = userGraphId || FALLBACK_GRAPH_ID;
  
  // Results state for each feature
  const [batchResult, setBatchResult] = useState<FeatureResult>({ loading: false, success: false });
  const [inferenceResult, setInferenceResult] = useState<FeatureResult>({ loading: false, success: false });
  const [clusterResult, setClusterResult] = useState<FeatureResult>({ loading: false, success: false });
  const [insightResult, setInsightResult] = useState<FeatureResult>({ loading: false, success: false });
  const [inventionResult, setInventionResult] = useState<FeatureResult>({ loading: false, success: false });
    ...
    try {
      ...
      if (data.batch_id) {
        ...
        const pollProgress = async () => {
          try {
            const statusRes = await fetch(`/api/v1/batch/${data.batch_id}/status`);
            ...
            setBatchResult({ loading: false, success: false, error: 'Progress check failed' });
          }
        };
        
        pollProgress();
      } else {
        setBatchResult({ loading: false, success: false, error: data.detail || 'Upload failed' });
      }
    } catch (e: any) {
  ...
  const StatusIcon = ({ result }: { result: FeatureResult }) => {
    if (result.loading) return <Loader2 className="h-4 w-4 animate-spin text-cyan-400" />;
    if (result.success) return <CheckCircle className="h-4 w-4 text-emerald-400" />;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-200">Batch Upload</span>
            </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-200">Cross-doc Inference</span>
            </div>
          ...
          {inferenceResult.data?.inferences && (
            <div className="mt-2 max-h-20 overflow-y-auto">
              {inferenceResult.data.inferences.slice(0, 3).map((inf: InferenceLink, i: number) => (
                <p key={i} className="text-[9px] text-slate-400 truncate">
                  {inf.source.slice(0,8)}↔{inf.target.slice(0,8)} ({(inf.similarity * 100).toFixed(0)}%)
                </p>
              ))}
              {inferenceResult.data.inferences_found > 3 && (
                <p className="text-[9px] text-slate-500">+{inferenceResult.data.inferences_found - 3} more</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-200">Clustering</span>
            </div>
          ...
          {clusterResult.data?.clusters && (
            <div className="mt-2 max-h-20 overflow-y-auto">
              {clusterResult.data.clusters.slice(0, 3).map((c: Cluster, i: number) => (
                <p key={i} className="text-[9px] text-slate-400 truncate">
                  {c.label} ({c.size} nodes)
                </p>
              ))}
              {clusterResult.data.n_clusters > 3 && (
                <p className="text-[9px] text-slate-500">+{clusterResult.data.n_clusters - 3} more</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-200">Hidden Insights</span>
            </div>
          ...
          {insightResult.data?.insights && (
            <div className="mt-2 max-h-20 overflow-y-auto">
              {insightResult.data.insights.slice(0, 3).map((ins: Insight, i: number) => (
                <p key={i} className="text-[9px] text-slate-400 truncate">
                  💡 {ins.title}
                </p>
              ))}
              {insightResult.data.insights_found > 3 && (
                <p className="text-[9px] text-slate-500">+{insightResult.data.insights_found - 3} more</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-200">Image Understanding</span>
            </div>
          </div>
          ...
          <p className="text-[9px] text-slate-600 italic">
            Images are auto-processed during batch upload when vision API is configured.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-slate-200">Self-Inventing</span>
            </div>
          ...
          {inventionResult.data?.inventions && (
            <div className="mt-2 max-h-20 overflow-y-auto">
              {inventionResult.data.inventions.slice(0, 3).map((inv: Invention, i: number) => (
                <p key={i} className="text-[9px] text-slate-400 truncate">
                  ✨ {inv.title} ({(inv.confidence * 100).toFixed(0)}%)
                </p>
              ))}
              {inventionResult.data.inventions_created > 3 && (
                <p className="text-[9px] text-slate-500">+{inventionResult.data.inventions_created - 3} more</p>
              )}
            </div>
          )}
        </div>
      ...
      {/* Error Display */}
      {(batchResult.error || inferenceResult.error || clusterResult.error || insightResult.error || inventionResult.error) && (
