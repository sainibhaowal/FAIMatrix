      ...
      fetch('/api/v1/me')
        ...
        .then(data => {
          if (data.graph_id && data.graph_id !== graphId) {
            console.log('[UserContext] Auto-healing stale Graph ID:', data.graph_id);
            // Update localStorage
            try {
              window.localStorage.setItem('faim.universe_graph_id', data.graph_id);
              window.localStorage.setItem('faim_universe_graph_id', data.graph_id);
...
export function useUserIds() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated && (projectId || graphId)) {
      console.warn('[FAIM] Using dev mode IDs. In production, users should be authenticated.');
    }
