                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      active
                        ? "bg-primary-500/15 text-primary-200 border border-primary-500/30 shadow-sm"
...
export default function EmbeddingProvidersPage() {
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    try {
      const { models, dimension } = await discoverEmbeddingModels(
        formData.baseUrl,
      ...
      if (models.length > 0) {
        ...
        if (dimension) {
          setFormData((prev) => ({ ...prev, dimension }));
        }
        setFormStep("models");
      } else {
  ...
  const handleAddProvider = useCallback(async () => {
    ...
    try {
      const id = await addEmbeddingProvider(
        formData.name,
        formData.type,
        formData.baseUrl,
        formData.apiKey || undefined,
      ...
      if (id) {
        setFormData({ name: "", type: "local", baseUrl: "", apiKey: "", model: "", dimension: 1024 });
        setDiscoveredModels([]);
        setSelectedModel("");
        setFormStep("config");
  ...
  const handleTestProvider = useCallback(
    async (providerId: string) => {
      await testEmbeddingProvider(providerId);
    },
    [testEmbeddingProvider],
  );
  ...
  const getTypeInfo = (type: string) => {
    switch (type) {
      case "local":
        return { label: "Local (bge-m3)", icon: <Cpu size={16} />, accent: "#10b981", free: true };
      case "openai":
        return { label: "OpenAI", icon: <Zap size={16} />, accent: "#f59e0b", free: false };
      case "custom":
        return { label: "Custom Endpoint", icon: <Shield size={16} />, accent: "#8b5cf6", free: true };
      default:
        return { label: "Unknown", icon: <AlertCircle size={16} />, accent: "#64748b", free: false };
    }
  };
  ...
  return (
    <div className="relative space-y-6 pb-8 text-slate-100 px-1">
      ...
      <GlassHeader
        ...
        subtitle="Semantic vector models for evolution & retrieval"
        icon={Cpu}
        actions={
          <Button
            size="sm"
        ...
        <div className="relative border-b border-white/8 pb-4 flex items-center justify-between">
          <div>
            <p className="font-mono text-[10px] font-bold uppercase tracking-[0.28em] text-cyan-400">
              SEMANTIC VECTOR PROVIDER MANAGEMENT
            </p>
          ...
          <div className="relative rounded-[14px] border border-dashed border-white/10 p-12 flex flex-col items-center gap-4 text-center bg-white/[0.01]">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 max-w-md mb-4 leading-relaxed">
                Add your first embedding provider (BAAI/bge-m3 local, OpenAI, or custom
                endpoint) to enable semantic evolution, retrieval, and invention.
              </p>
              <Button
                size="sm"
                leftIcon={<Plus size={14} />}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        {React.cloneElement(typeInfo.icon, { style: { color: typeInfo.accent } })}
                        <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-2">
                          {typeInfo.label}
                        </p>
                        {typeInfo.free && (
                          <span className="inline-block px-2 py-0.5 rounded bg-emerald-500/20 border border-emerald-500/30 text-[9px] font-bold text-emerald-400 uppercase tracking-wider">
                            Free
                          </span>
                        )}
                        {!typeInfo.free && (
                          <span className="inline-block px-2 py-0.5 rounded bg-amber-500/20 border border-amber-500/30 text-[9px] font-bold text-amber-400 uppercase tracking-wider">
                            Paid
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-slate-700/50">
                    {!provider.isActive && (
                      <Button
                        ...
                        variant="outline"
                        className="flex-1"
                        onClick={() => setActiveEmbeddingProvider(provider.id)}
                      >
                        Set Active
                      </Button>
                    )}
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-cyan-500/30 text-cyan-500 hover:bg-cyan-500/5 flex-1"
                    ...
                    <Button
                      ...
                      onClick={() => removeEmbeddingProvider(provider.id)}
                    >
                      Delete
                    </Button>
                  </div>
                  ...
                  <div className="space-y-4">
                            ...
                            <label key={t} className="flex-1 cursor-pointer">
                              ...
                              <div
                                className={`px-3 py-2 rounded-lg border text-center text-[10px] font-bold uppercase tracking-widest transition-all cursor-pointer ${
                                  formData.type === t
                                    ? `bg-${info.accent.replace("#", "")}20 border-${info.accent}50 text-${info.accent.replace("#", "")}300`
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder={
                          formData.type === "local"
                            ? "http://localhost:8000 (or leave empty for built-in)"
                      ...
                      {formData.type === "local" && (
                        <p className="text-[9px] text-slate-500 mt-1.5">
                          Built-in BAAI/bge-m3 runs locally. Use your local inference server URL,
                          or leave empty to use the in-process model (coming soon).
                        </p>
                      )}
                      {formData.type === "local" && (
                        <p className="text-[9px] text-slate-500 mt-1.5">
                    ...
                    {formData.type === "custom" && (
                      <div>
                        ...
                        <input
                          ...
                          placeholder="e.g., bge-m3, text-embedding-3-small"
                          className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-primary-500/50"
                          disabled={formLoading}
                        />
                      </div>
                    ...
                    {formData.type === "custom" && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({ ...formData, dimension: parseInt(e.target.value) || 1024 })
                          }
                          placeholder="1024"
                          className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-primary-500/50"
                          disabled={formLoading}
