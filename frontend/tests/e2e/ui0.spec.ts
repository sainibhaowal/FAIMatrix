...
test.describe("UI0: Visual Shell", () => {
  ...
  test("UI0-G1: Landing renders and mobile layout is responsive", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("UI0-G2: Login page renders without console errors", async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on("console", (msg) => {
  ...
  test("UI0-G3: Dashboard renders with mock data and activity", async ({
    page,
  }) => {
  ...
  test("UI0-G4: Graph page shows FIG view with mock data", async ({ page }) => {
    ...
    const inspectorPanel = page.locator(
      '[class*="Inspector"], [class*="inspector"], [class*="sidebar"]',
    );
